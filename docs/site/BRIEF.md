...
Objective: help a developer understand the portable Project Observatory, inspect a synthetic exposure workflow, and give an agent the installation/onboarding task.
...
- SITE-01: First-time builder opens `/`, learns that the product observes explicitly enrolled projects locally, and reaches agent onboarding. Success: purpose and CTA visible in the first desktop viewport; HTML remains useful without JS.
...
- SITE-03: Developer reads the supported capabilities and release boundary before adopting. Missing cloud adapters, MCP and scheduling are linked to the migration plan, never presented as available.
